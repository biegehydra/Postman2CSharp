...
{
    public class GooglePlaceApiClient : IGooglePlaceApiClient
    {
        private readonly HttpClient _httpClient;
        private readonly string _apiKey;
        public GooglePlaceApiClient(HttpClient httpClient, IConfiguration config)
        {
        ...
        /// <summary>
        /// A Find Place request takes a text input and returns a place. The input can be any kind of Places text 
        /// data, such as a name, address, or phone number. The request must be a string. A Find Place request using 
        ...
        /// The service responds with a list of places matching the text string and any location bias that has been 
        /// set.
        /// The service is especially useful for making [ambiguous address](https://developers.google.com/maps/documentation/geocoding/best-practices) 
        /// queries in an automated system, and non-address components of the string may match businesses as well 
        /// as addresses. Examples of ambiguous address queries are incomplete addresses, poorly formatted addresses, 
        ...
        /// <summary>
        /// The Place Photo service, part of the Places API, is a read- only API that allows you to add high quality 
        /// photographic content to your application. The Place Photo service gives you access to the millions of 
        /// photos stored in the Places database. When you get place information using a Place Details request, 
        /// photo references will be returned for relevant photographic content. Find Place, Nearby Search, and 
        /// Text Search requests also return a single photo reference per place, when relevant. Using the Photo 
        /// service you can then access the referenced photos and resize the image to the optimal size for your 
        /// application.
        /// Photos returned by the Photo service are sourced from a variety of locations, including business owners 
        /// and user contributed photos. In most cases, these photos can be used without attribution, or will have 
        /// the required attribution included as a part of the image. However, if the returned photo element includes 
        /// a value in the html_attributions field, you will have to include the additional attribution in your 
        ...
        /// Instead of searching for a specific location, a user can type in a categorical search, such as "pizza 
        /// near New York" and the service responds with a list of suggested queries matching the string. As the 
        ...
        /// <summary>
        /// The Place Autocomplete service is a web service that returns place predictions in response to an HTTP 
        /// request. The request specifies a textual search string and optional geographic bounds. The service can 
        /// be used to provide autocomplete functionality for text-based geographic searches, by returning places 
        /// such as businesses, addresses and points of interest as a user types.>Note: You can use Place Autocomplete 
        /// even without a map. If you do show a map, it must be a Google map. When you display predictions from 
        /// the Place Autocomplete service without a map, you must include the ['Powered by Google'](https://developers.google.com/maps/documentation/places/web-service/policies#logo_requirementshttps://developers.google.com/maps/documentation/places/web-service/policies#logo_requirements) 
        /// logo.
        /// The Place Autocomplete service can match on full words and substrings, resolving place names, addresses, 
        /// and plus codes. Applications can therefore send queries as the user types, to provide on-the-fly place 
        /// predictions.
        /// The returned predictions are designed to be presented to the user to aid them in selecting the desired 
