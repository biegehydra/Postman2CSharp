...
{
    public class SystemTextJsonApiClient : ISystemTextJsonApiClient
    {
        ...
        /// 
        /// </summary>
        public async Task<AutocompleteResponse> Autocomplete(AutocompleteParameters queryParameters)
        {
            var parametersDict = queryParameters.ToDictionary();
            parametersDict.Add($"key", _apiKey);
            var queryString = QueryHelpers.AddQueryString($"autocomplete/json", parametersDict);
            return await _httpClient.GetFromJsonAsync<AutocompleteResponse>(queryString);
        }
    }
}