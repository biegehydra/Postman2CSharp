...
namespace SystemTextJson
{
    ...
    public class AutocompleteParameters : IQueryParameters
    {
        ...
        /// components to filter by up to 5 countries. Countries must be passed as a two character, ISO 3166-1 
        /// Alpha-2 compatible country code. For example: `components=country:fr` would restrict your results to 
        /// places within France. Multiple countries must be passed as multiple `country:XX` filters, with the 
        /// pipe character `|` as a separator. For example: 
        /// `components=country:us|country:pr|country:vi|country:gu|country:mp` would restrict your results to 
        /// places within the United States and its unincorporated organized territories.>Note: If you receive 
        /// unexpected results with a country code, verify that you are using a code which includes the 
        /// countries, dependent territories, and special areas of geographical interest you intend.  You can 
        /// find code information at Wikipedia: List of ISO 3166 country codes or the ISO Online Browsing 
        /// Platform
        ///  
        /// </summary>
        ...
        /// For example, if the input is `Google` and the offset is 3, the service will match on `Goo`. The 
        /// string determined by the offset is matched against the first word in the input term only. For 
        /// example, if the input term is `Google abc` and the offset is 3, the service will attempt to match 
        /// against `Goo abc`. If no offset is supplied, the service will use the whole term. The offset should 
        ...
        /// `latitude,longitude`. >When using the Text Search API, the `location` parameter may be overriden if 
        /// the `query` contains an explicit location such as `Market in Barcelona`.
        ///  
        /// </summary>
        ...
        /// `types` parameter. This parameter specifies a type or a type collection, as listed in [Place 
        /// Types](/maps/documentation/places/web-service/supported_types). If nothing is specified, all types 
        /// are returned.
        /// 
        /// For the value of the `types` parameter you can specify either:
        /// * Up to five values 
        /// from [Table 1](/maps/documentation/places/web-service/supported_types#table1) or [Table 
        /// 2](/maps/documentation/places/web-service/supported_types#table2). For multiple values, separate 
        /// each value with a `|` (vertical bar). For example:
        /// 
        ///   `types=book_store|cafe`
        /// 
        /// * Any supported 
        /// filter in [Table 3](/maps/documentation/places/web-service/supported_types#table3). You can safely 
        /// mix the `geocode` and `establishment` types. You cannot mix type collections (`address`, `(cities)` 
        /// or `(regions)`) with any other type, or an error occurs.
        /// 
        /// The request will be rejected with an 
        /// `INVALID_REQUEST` error if:
        /// 
        /// * More than five types are specified.
        /// * Any unrecognized types are 
        /// present.
        /// * Any types from in [Table 
